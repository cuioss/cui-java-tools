...
public final class UnifiedValidationPipeline implements HttpSecurityValidator {
    ...
    private String sanitizeForLogging(@NonNull String input, @NonNull ValidationType type) {
        return switch (type) {
            ...
            case URL_PATH, BODY, PARAMETER_NAME, HEADER_NAME, COOKIE_NAME -> input; // These are generally safe to log
        };
    }
}
----
...
public class ValidationFactory {
    ...
    public HttpSecurityValidator createValidator(@NonNull ValidationType type) {
        ConfigStageProvider config = switch (type) {
            ...
            case PARAMETER_NAME -> new URLParameterNameConfig(baseConfig);
            case PARAMETER_VALUE -> new URLParameterValueConfig(baseConfig);
            case HEADER_NAME -> new HTTPHeaderNameConfig(baseConfig);
            case HEADER_VALUE -> new HTTPHeaderValueConfig(baseConfig);
            case BODY -> new HTTPBodyConfig(baseConfig);
            case COOKIE_NAME -> new CookieNameConfig(baseConfig);
            case COOKIE_VALUE -> new CookieValueConfig(baseConfig);
