...
**MOVED**: Known quality issues have been moved to a dedicated document for better organization and tracking.

**See**: link:bugs.md[HTTP Security Verification - Known Quality Issues]

This separate document contains:
- QI-1: Imprecise Test Assertions with OR Conditions
- QI-2: ✅ RESOLVED: HTTP Protocol Layer Scope Defined (HTML/JS escapes moved to application layer)
- QI-3: ✅ RESOLVED: JavaScript Escapes Correctly Scoped to Application Layer
- QI-4: Test Generator Reliability Issues
- QI-5: Flaky Test Dependencies on Random Seeds

Each issue includes detailed problem analysis, security impact assessment, affected components, and implementation strategies for resolution.
