 ...
 * These BitSets are immutable after initialization and thread-safe for concurrent reads.
...
public final class CharacterValidationConstants {
    ...
    static {
        ...
        unreserved.set('_');
        unreserved.set('~');
        RFC3986_UNRESERVED = (BitSet) unreserved.clone();
        
        // Initialize RFC3986_PATH_CHARS
        BitSet pathChars = (BitSet) unreserved.clone();
        pathChars.set('/');
        pathChars.set('@');
        pathChars.set(':');
        // sub-delims for path: "!" / "$" / "&" / "'" / "(" / ")" / "*" / "+" / "," / ";" / "="
        "!$&'()*+,;=".chars().forEach(pathChars::set);
        RFC3986_PATH_CHARS = (BitSet) pathChars.clone();
        
        // Initialize RFC3986_QUERY_CHARS
        BitSet queryChars = (BitSet) unreserved.clone();
        queryChars.set('?');
        queryChars.set('&');
        queryChars.set('=');
        // sub-delims for query
        "!$'()*+,;".chars().forEach(queryChars::set);
        RFC3986_QUERY_CHARS = (BitSet) queryChars.clone();
        ...
        RFC7230_HEADER_CHARS = (BitSet) headerChars.clone();
    }
    
    /**
     * Returns a defensive copy of the character set for the given validation type.
     * This ensures the original BitSets remain immutable.
     */
    public static BitSet getCharacterSet(ValidationType type) {
        return switch (type) {
            case URL_PATH -> (BitSet) RFC3986_PATH_CHARS.clone();
            case URL_PARAMETER -> (BitSet) RFC3986_QUERY_CHARS.clone();
            case HTTP_HEADER -> (BitSet) RFC7230_HEADER_CHARS.clone();
            default -> (BitSet) RFC3986_UNRESERVED.clone();
        };
    }
}
----
...
public final class CharacterValidationStage implements HttpSecurityValidator {
    ...
    public CharacterValidationStage(UrlSecurityConfig config, ValidationType type) {
        ...
        // Get a defensive copy of the appropriate character set
