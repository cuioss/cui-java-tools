...
/**
 * QI-5: Basic generator test for SqlInjectionAttackGenerator.
 * 
 * <p>
...
class SqlInjectionAttackGeneratorTest {

    @ParameterizedTest
    @TypeGeneratorSource(value = SqlInjectionAttackGenerator.class, count = 10)
    @DisplayName("Generator should produce valid non-null SQL injection attack patterns")
    void shouldGenerateValidOutput(String generatedValue) {
        ...
        assertTrue(generatedValue.length() > 3, "SQL injection patterns should be meaningful (more than 3 characters)");
    }
}