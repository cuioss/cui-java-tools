...
/**
 * QI-5: Basic generator test for ValidHTTPBodyContentGenerator.
 *
 * <p>
...
class ValidHTTPBodyContentGeneratorTest {

    @ParameterizedTest
    @TypeGeneratorSource(value = ValidHTTPBodyContentGenerator.class, count = 10)
    @DisplayName("Generator should produce valid non-null HTTP body content")
    void shouldGenerateValidOutput(String generatedValue) {
        assertNotNull(generatedValue, "Generator must not produce null values");
        assertFalse(generatedValue.isEmpty(), "Generator should produce non-empty HTTP body content");
        assertTrue(generatedValue.length() > 2, "HTTP body content should be meaningful (more than 2 characters)");
    }
}